word_list_rus = ['дамир', 'илья', 'шкила', 'год', 'человек', 'время', 'дело', 'жизнь', 'день', 'рука', 'раз', 'работа', 'слово', 'место', 'лицо',
...
#Возвращает рандомное слово в верхнем регистре
def get_word():
    global word_list_rus
    from random import choice
    word = choice(word_list_rus).upper()
    return word

# функция получения текущего состояния. Tries кол-во попыток: 6 - начало, а 0 - конец
...
def play(word, t):
    ...
    while (word_completion != word) and (tries > 0):
        ...
        print(display_hangman(tries)) #Рисуем человечка
        print(*word_completion) #Рисуем слово
        print('-Какую букву предложите или может сразу скажете слово?')
        w = input('-').upper() #Получем букву (заглавную)


        if len(w) == 1: #Если ввели только одну букву
            if w in guessed_letters: #если букву уже вводили
                print('-Простите, но вы тупой и эту букву уже вводили( Давате еще раз!')
                continue

            elif w in word: #Если буква есть в слове
                print('-Да эта буква есть в слове')
                guessed_letters.append(w) #Добавляем букву в уже введенные
                list_of_is = find_letter(word, w) #Ищем номер буквы в слове

                for i in range(len(list_of_is)):
                    i = list_of_is[i]
                    i = int(i)
                    word_completion = word_completion[:i] + w + word_completion[i + 1:]  #Вставляем в ____ угаданную букву

            elif w in rus_upper_alphabet: #Если буквы нет в слове, но она есть в Рус алфавите
                print('-Увы, но этой буквы в слове нет')
                guessed_letters.append(w)  # Добавляем букву в уже введенные
                t_peice +=1
                continue
            ...
            #ВЫ еблан!
                ...
                print('-Увы, но вы еблан. Приносим вам бан...')
                sleep(3)
                print('processing 10%...')
                sleep(10)
                print('processing 30%...')
            ...
            print('-Увы, но это не то слово')
            t_peice +=1
            continue

        elif w == word:
            print('-Ураааа! Поздравляем вы отгадали слово!')
            word_completion = word
            break


        else:
            print('-Простите, но вы сотворили хуйню. Мы вас баним, бип-буп')

    if word_completion == word:
        print('Уррааааа! Вы выйграли ТЫНДЫН. Ваше слово было:', word)

    else:
        print(display_hangman(tries))  # Рисуем человечка
        print('-Вы ужасен. Вы виновны в смерти человека. Больше не играйте', 'Вот ваше слово:', word)



print('-Приветствуем вас в Виссилице. Здесь вам предстоит угадать слово, но перед этим выберете кол-во попыток. Рекомендуем выбирать числа кратные 6')
tries = input('-')
while not tries.isdigit():
    print('-Рекомндуется выбрать число. Иначе играть', tries, 'раз не выйдет')
    tries = input('-')
tries = int(tries)


play(get_word(), tries)    #Проигрывание